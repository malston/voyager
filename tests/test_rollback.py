...
def mock_env_setup():
    ...
    with patch('git.Repo') as mock_git_repo, \
         patch('voyager.commands.rollback.check_git_repo', return_value=True), \
         patch('voyager.commands.rollback.get_repo_info', return_value=('test-owner', 'test-repo')), \
         patch('voyager.commands.rollback.GitHubClient') as mock_github_client, \
         patch('voyager.commands.rollback.find_version_file') as mock_find_version, \
         patch('voyager.commands.rollback.extract_version') as mock_extract_version, \
         patch('voyager.commands.rollback.VersionUpdater') as mock_version_updater, \
         patch('voyager.commands.rollback.update_version_in_init') as mock_update_init:
        ...
        github_instance.create_release.return_value = {'html_url': 'https://github.com/test/test/releases/rollback-v1.0.0'}
        
        # Mock version file finding and extraction
        mock_find_version.return_value = ('/fake/path/pyproject.toml', r'version\s*=\s*[\'"](?P<version>[^\'"]*)[\'"]')
...
def test_rollback_with_version_file(mock_env_setup):
    ...
    mock_env_setup['find_version'].return_value = ('/fake/path/pyproject.toml', r'version="([^"]*)"')
    ...
    with runner.isolated_filesystem():
        ...
        with patch('click.confirm', return_value=True):
            ...
            result = runner.invoke(
                rollback, 
                ['--tag', 'v0.9.0', '--version-file', 'pyproject.toml', '--version-branch', 'version'],
...
def test_rollback_with_version_branch_update(mock_env_setup):
    ...
    with runner.isolated_filesystem():
        ...
        with patch('click.confirm', return_value=True):
            ...
            result = runner.invoke(
                rollback,
                ['--tag', 'v0.9.0', '--version-branch', 'version'],
                catch_exceptions=True
            )
